...
private fun buildWorkspacePrompt(workspace: WorkspaceEntity, cwd: String? = null): String = buildString {
    ...
    appendLine("  - `workspace_list`: list files and directories.")
    appendLine("  - `workspace_stat`: inspect a file or directory.")
    appendLine("  - `workspace_glob`: find paths by glob pattern.")
    appendLine("  - `workspace_grep`: search text in files.")
    ...
    appendLine("  - `workspace_mkdir`: create directories inside /workspace.")
    appendLine("  - `workspace_delete`: delete files or directories inside /workspace.")
    appendLine("  - `workspace_rename`: rename or move files/directories inside /workspace.")
    appendLine("  - `workspace_shell`: run shell commands (the files area is mounted at /workspace).")
    appendLine("- Prefer `workspace_list`, `workspace_stat`, `workspace_glob`, and `workspace_grep` for discovery tasks; use `workspace_shell` for full Unix workflows, and prefer `workspace_edit_file` for targeted edits over rewriting whole files.")
